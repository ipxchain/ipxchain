...
import (
	...
	kbkeys "github.com/ipxchain/ipxchain/crypto/keys"
	...
	"github.com/ipxchain/ipxchain/scp/staking/client/cli"
)
...
// GenTxCmd builds the truth gentx command.
// nolint: errcheck
func GenTxCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "gentx",
		Short: "Generate a genesis tx carrying a self delegation",
		Long: fmt.Sprintf(`This command is an alias of the 'truth tx create-validator' command'.
...
`, defaultAmount, defaultCommissionRate, defaultCommissionMaxRate, defaultCommissionMaxChangeRate, defaultMinSelfDelegation),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if info.GetType() == kbkeys.TypeOffline || info.GetType() == kbkeys.TypeMulti {
				fmt.Println("Offline key passed in. Use `truthcli tx sign` command to sign:")
