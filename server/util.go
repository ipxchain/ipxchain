...
import (
	...
	"github.com/ipxchain/ipxchain/server/config"
...
func interceptLoadConfig() (conf *cfg.Config, err error) {
	...
	// create a default truth config file if it does not exist
	gaiaConfigFilePath := filepath.Join(rootDir, "config/truth.toml")
